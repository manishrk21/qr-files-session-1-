...
// Reuses the same status-transition logic as the board, plus shows customer info,
// payment details, and full status history — none of which fit on the card view.
...
export default function AdminOrderDetailPage() {
  ...
  const fetchOrder = useCallback(async () => {
    ...
    else toast.error(data.error?.message ?? 'Could not load order');
    setLoading(false);
  }, [params.orderId]);
    ...
    return (
      <div className="p-6">
        ...
        <Link
          ...
          className="text-amber-400 text-sm underline mt-2 inline-block"
        >
          ← Back to orders
        </Link>
      </div>
  ...
  return (
    <div className="p-6 max-w-3xl">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <span className={`text-xs font-semibold px-3 py-1.5 rounded-full border flex items-center gap-1.5 ${STATUS_COLOR[order.status]}`}>
            ...
            <div className="space-y-2.5">
              {order.items.map((item) => (
                <div key={item.id} className="flex justify-between text-sm">
                  ...
                  <span className="text-gray-400">₹{Number(item.subtotal).toFixed(2)}</span>
                </div>
              ))}
            </div>
            ...
            <div className="space-y-3">
              {order.statusHistory.map((h, i) => (
                <div key={i} className="flex items-center gap-3 text-sm">
                  ...
                  <span className="text-gray-300 capitalize flex-1">{h.status.replace('_', ' ')}</span>
        ...
        <div className="space-y-5">
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5">
            ...
            <div className="flex items-center justify-between text-sm">
              ...
              <span className={`font-medium ${order.payment.status === 'paid' ? 'text-green-400' : 'text-gray-300'}`}>
          ...
          {!isTerminal && transitions.length > 0 && (
            <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5 space-y-2">
              ...
              {transitions.filter((t) => t !== 'cancelled').map((nextStatus) => (
                ...
                >
                  {updating ? <Loader2 size={14} className="animate-spin mx-auto" /> : ACTION_LABEL[nextStatus] ?? nextStatus}
                </button>
              ))}
